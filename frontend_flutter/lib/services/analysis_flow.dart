...
/// Runs `/analyze-image` on raw JPEG bytes and shows the same UI as the Photo tab.
Future<void> runDeviceCameraAnalysisFromBytes(
  BuildContext context, {
  required List<int> imageBytes,
  String fileName = 'camera_capture.jpg',
}) async {
  ...
  try {
    ...
    final results = await analysisService.analyzeImageBytes(imageBytes, filename: fileName);

    if (!context.mounted) return;
    navigator.pop();

    history.addFromResults(AnalysisKind.photo, results, sourceFileName: fileName);
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (ctx) => AnalysisResultsSheet(results: results),
    );
